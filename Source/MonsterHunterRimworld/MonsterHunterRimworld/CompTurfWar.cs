...
namespace MonsterHunterRimworld
{
    public class CompTurfWar : ThingComp
    {
        ...
        public override void CompTickRare()
        {
            ...
            List<Pawn> possibleTargetsToFight = map.mapPawns.AllPawnsSpawned
                .Where(p => p.RaceProps.Animal && p.Faction == null && !p.IsFighting() && !p.Downed && !p.Dead && !WyvernUtility.IsSameSpecies(p, largeMonster) && p.Position.DistanceTo(largeMonster.Position) < Props.maxFightRange && p.TryGetComp<CompTurfWar>() != null)
                .ToList(); if (possibleTargetsToFight.Count == 0) return;
